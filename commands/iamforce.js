...
module.exports = async (bot, message, args, Discord) => {
  if (message.member.roles.some(r => ["Captain", "Commander", "Lieutenant"].includes(r.name))) {
    ...
    const m = await message.channel.send(`Force Adding **"${firstname} ${lastname}"** as a Recruit, Give me a sec...`);

    if (!message.mentions.members.first().roles.some(r => ["Visitor"].includes(r.name))) {
      return (
        m.edit(`This command can only be used on Visitors.`),
        message.react("❌"),
        setTimeout(() => {
          message.delete();
          m.delete();
    ...
    let embed = {
      ...
      author: {
        name: `${firstname} ${lastname} is now verified as a Recruit!`
      },
      fields: [
        {
          name: "Ingame Name:",
    ...
    await message.mentions.members.first().addRole(message.member.guild.roles.find(r => r.name === "Recruit"));
