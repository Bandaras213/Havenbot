  ...
  try {
    await fetch("https://xivapi.com/character/search?name=" + args.join("%20") + "&server=" + server + `&key=${process.env.XIVAPIKEY}`, {
      ...
      .then(async res => {
        ...
        await fetch("https://xivapi.com/character/" + lodeID[0].ID + `?key=${process.env.XIVAPIKEY}`, {
          method: "post"
        })
          .then(sear => sear.json())
          .then(async sear => {
            ...
            await fetch(`https://xivapi.com/Title/` + `${titleID}` + `?key=${process.env.XIVAPIKEY}`, {
              method: "post"
            })
              ...
              .then(async apititle => {
                ...
                const ctx = canvas.getContext("2d");
                const portrait = await Canvas.loadImage(searCharacter.Portrait);
