            ...
            .then(async res => {
                ...
                await fetch("https://xivapi.com/character/" + lodeID[0].ID + `?key=${bot.config.xivapikey}`)
                    ...
                    .then(async sear => {
                        ...
                        let MinionFilter = JSON.parse(fs.readFileSync(Database, 'utf8')).Minions.filter(ID => ID.ID == 0);
                        let MountFilter = JSON.parse(fs.readFileSync(Database, 'utf8')).Mounts.filter(ID => ID.ID == 1);
                        let RaceFilter = JSON.parse(fs.readFileSync(Database, 'utf8')).Races.filter(ID => ID.ID == searCharacter.Race - 1);
                        let TribesFilter = JSON.parse(fs.readFileSync(Database, 'utf8')).Tribes.filter(ID => ID.ID == searCharacter.Tribe - 1);
                        ...
                        if (searCharacter.GrandCompany == null) {
                            GCNameFilter = JSON.parse(fs.readFileSync(Database, 'utf8')).GrandCompanys.filter(ID => ID.ID == 0);
                            GCRankFilter = GCNameFilter[0].Ranks.filter(RankID => RankID.RankID == 0);
                            //else use the grand company of the character
                        } else {
                            GCNameFilter = JSON.parse(fs.readFileSync(Database, 'utf8')).GrandCompanys.filter(ID => ID.ID == searCharacter.GrandCompany.NameID);
                            GCRankFilter = GCNameFilter[0].Ranks.filter(RankID => RankID.RankID == searCharacter.GrandCompany.RankID - 1);
                        };
                        let Jobs = JSON.parse(fs.readFileSync(Database, 'utf8')).ClassJobs
